...
class SolubilityGuider:
    ...
    def compute_saliency(self, embeddings, attention_masks, saliency_grads):
        ...
        """
        optim = torch.optim.SGD([saliency_grads], lr=1) # 1x the gradient value = gradient value itself i think
        ...
        saliency = saliency_grads.abs().sum(dim=-1)  # summation across hidden dim. Abs value for directionality only
    ...
    def determine_edit_positions(self, saliency_map, solubility_preds):
        ...
        """
        # Selection procedure to determine top-K low-value residues (edit positions)
        ...
        #_, topk_edit_pos = torch.topk(probabilities, nonzero)
    ...
    def optimized_sampling(self, og_logits, og_hidden, attention_mask, n_steps=None, use_stoch_term=False):
        ...
        h_new = og_hidden + delta.data # Final hidden state update
