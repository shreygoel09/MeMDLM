...
class Diffusion(L.LightningModule):
    ...
    def scheduled_q_xt(self, x, mask_rate):
        ...
        tot_tokens = x.numel() #- (x == self.tokenizer.pad_token_id).sum().item()
    ...
    def _forward_pass_diffusion(self, x0, attention_mask, batch_idx):
        ...
        if self.curriculum:
            ...
            move_chance = torch.empty_like(x0, dtype=torch.float, device=x0.device).uniform_(0.1, 0.2)
            xt = self.scheduled_q_xt(x0, move_chance)
        else:
            t = self._sample_t(x0.shape[0], x0.device)
            sigma, dsigma = self.nosie(t)
    ...
    def _loss(self, x0, attention_mask, batch_idx):
        ...
        if self.curriculum:
            num_masked = (x0 == self.mask_index).sum().item()
            nlls = loss * num_masked
            token_nll = nlls.sum() / num_masked
        else:
