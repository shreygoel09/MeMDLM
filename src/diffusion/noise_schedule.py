...
def get_noise(config, min_mask_rate=None, max_mask_rate=None, increment=None, dtype=torch.float32):
...
class ScheduledCosine(Noise):
    def __init__(self, min_mask, max_mask, increment):
        self.min = min_mask
        self.max = max_mask
        self.increment = increment

    def rate_noise(self, t):
        return self.min + 0.5 * (self.max - self.min) * (1 - torch.cos(t))
        
    def total_noise(self, t):
        noise = (t * self.min) + (0.5 * t * (self.max - self.min)) - (0.5 * t * (self.max - self.min) * torch.sin(t))
        return -torch.log(torch.clamp(noise, min=1e-8))
