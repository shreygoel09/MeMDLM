...
#     #   solubilities.append([1 if residue.lower() else 0 for residue in sequence])
...
def collate_fn(batch, tokenizer):
    sequences = [item['Sequence'] for item in batch]
    tokens = tokenizer(sequences, return_tensors='pt', truncation=True, padding='max_length', max_length=1024)
    return {
      'input_ids': tokens['input_ids'],
      'attention_mask': tokens['attention_mask']
    }
...
#### MDLM BASE IMPLEMENTATION CODE: IRRELEVANT ####
