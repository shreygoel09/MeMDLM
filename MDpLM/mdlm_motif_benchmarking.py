import torch
import torch.nn.functional as Fxs
import sys
import pandas as pd
...
def mdlm_motif_benchmark(config):
    ...
    test_sequences = pd.read_csv(path + "/data/membrane/test.csv")['Sequence'].tolist()[:5]
    ...
    mdlm_model = Diffusion(config, tokenizer=tokenizer) # should autoload the checkpoint with config
    ...
    for generate_case in ['uppercase', 'lowercase']:
        ...
        for original_sequence in tqdm(test_sequences, desc=f"scaffolding ({generate_case}): "):
            ...
            generated_sequence = generate_scaffold_mdlm(original_sequence, generate_case, tokenizer, mdlm_model)
            print(generated_sequence)
            perplexity = mdlm_model.compute_generative_perplexity(generated_sequence)
            print(perplexity)
