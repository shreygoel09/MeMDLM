...
class Diffusion(L.LightningModule):
  ...
  def on_save_checkpoint(self, checkpoint):
    ...
    self.backbone.save_model(self.config.checkpointing.save_dir)
  ...
  def _subs_parameterization(self, logits, xt):
    ...
    # Prevent selection from eos/cls tokens as well
  ...
  def _compute_loss(self, batch, prefix):
    ...
    # Check if focused-masking is being used
    if 'mask' in batch:
      mask = batch['mask']
    else:
      mask = None
  ...
  def q_xt(self, x, move_chance):
    """Computes the noisy sample xt. Employs a maximal masking rate to prevent 
    fully masked batches during training..
    Args:
      x: int torch.Tensor with shape (batch_size,
          diffusion_model_input_length), input. 
      move_chance: float torch.Tensor with shape (batch_size, 1).
    Returns:
      xt: int torch.Tensor with shape of input x. Represents
      the noised (masked) latents.
    """
    actual_seq_length = (x != 1).sum(dim=1, keepdim=True)
    max_mask_length = (actual_seq_length * 0.90).long()
